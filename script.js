...
const STREAM_SERVERS = [
  { id: "zeno-1",   label: "ZENO 1",   url: "https://zeno1.waveradio.eu.org",   flag: "🇺🇸" },
  { id: "zeno-2",   label: "ZENO 2",   url: "https://zeno2.waveradio.eu.org",   flag: "🇺🇸" },
  { id: "listen-1", label: "LISTEN 1", url: "https://listen1.waveradio.eu.org", flag: "🇬🇧" },
  { id: "listen-2", label: "LISTEN 2", url: "https://listen2.waveradio.eu.org", flag: "🇬🇧" },
];
...
audio.preload = "none";
audio.volume = 0.8;
// audio.crossOrigin removed: listen servers lack CORS headers
...
const _artworkCache = new Map(); // key "artist::title" -> url | null

function toHttps(u){ return u && u.indexOf("http://")===0 ? "https://"+u.slice(7) : u; }

async function _itunesSearch(q){
  try{
    ...
    return url ? toHttps(url.replace("100x100bb","600x600bb")) : null;
  }catch(_){ return null; }
}

async function fetchArtwork(artist, title){
  const key = (artist+"::"+title).toLowerCase();
  if (_artworkCache.has(key)) return _artworkCache.get(key);
  const queries = [];
  const at = (artist+" "+title).trim();
  if (at) queries.push(at);
  if (title && queries.indexOf(title.trim())<0) queries.push(title.trim());
  if (artist && queries.indexOf(artist.trim())<0) queries.push(artist.trim());
  for (const q of queries){
    const u = await _itunesSearch(q);
    if (u){ _artworkCache.set(key, u); return u; }
  }
  _artworkCache.set(key, null);
  return null;
}
...
async function fetchNowPlaying(){
  try{
    ...
    const artwork = raw ? await fetchArtwork(artist, title) : null;
...
function renderNowPlaying(){
  ...
  // Disc artwork (home page) — toggle the persistent <img>, never wipe EQ/fallback
  const discImg = document.getElementById("discArtImg");
  if (discImg){
    if (nowPlaying.artwork){
      if (discImg.getAttribute("src") !== nowPlaying.artwork){
        discImg.onerror = () => { discImg.hidden = true; discImg.removeAttribute("src"); };
        discImg.onload  = () => { discImg.hidden = false; };
        discImg.src = nowPlaying.artwork;
      }
    } else {
      discImg.hidden = true;
  ...
  // Player bar art — toggle persistent <img> overlay
  const partImg = document.getElementById("playerArtImg");
  if (partImg){
    if (nowPlaying.artwork){
      if (partImg.getAttribute("src") !== nowPlaying.artwork){
        partImg.onerror = () => { partImg.hidden = true; partImg.removeAttribute("src"); };
        partImg.onload  = () => { partImg.hidden = false; };
        partImg.src = nowPlaying.artwork;
      }
    } else {
      partImg.hidden = true;
